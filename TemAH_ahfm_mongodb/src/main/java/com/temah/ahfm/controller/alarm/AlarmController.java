...
public class AlarmController extends BaseController<Alarm, String> {
    ...
    public RestResult terminateAlarm(@RequestBody String ids, HttpServletRequest request) {
        ...
        List<String> idListID = idList.stream().map(String::valueOf).collect(Collectors.toList());
    ...
    @PostMapping("/clear")
    public RestResult clearAlarm(@RequestParam(name = "id") String id, HttpServletRequest request) {
        if (id == null) {
            throw new IllegalArgumentException("缺少必填值");
        }
    ...
    @GetMapping("/get/{id}")
    public RestResult getAlarm(@PathVariable(name = "id") String id, HttpServletRequest request) {
        if (id == null) {
            throw new IllegalArgumentException("缺少必填值");
        }
        logger.info("获取告警: {}", id);
